...
async function optimizeBucketImages() {
  ...
  for (const file of targetFiles) {
    ...
    console.log(`--------------------------------------------------`);
    console.log(`🔍 Processing: ${originalFullPath} (${(originalSize / 1024).toFixed(2)} KB)`);
    ...
    // 5. Update database references in 'categories' table if applicable
    const { data: catRows, error: searchError } = await supabase
      .from('categories')
      .select('id, image')
      .like('image', `%${originalFullPath}%`);

    if (!searchError && catRows && catRows.length > 0) {
      for (const row of catRows) {
        if (row.image) {
          const updatedImageUrl = row.image.replace(originalFullPath, newFullPath);
          ...
          if (updateError) {
            console.error(`  ❌ Failed to update category ID ${row.id} image URL:`, updateError.message);
          } else {
            console.log(`  🔄 Updated Database Category [${row.id}]: ${row.image} -> ${updatedImageUrl}`);
            dbUpdatedCount++;
          }
        }
      }
...
optimizeBucketImages().catch((err) => {
