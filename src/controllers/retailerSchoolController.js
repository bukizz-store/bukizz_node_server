...
export class RetailerSchoolController {
  ...
  getConnectedSchools = async (req, res) => {
    try {
      ...
      const result = await retailerSchoolService.getConnectedSchools(
        retailerId,
        filters,
      );
  ...
  /**
   * Get all globally pending retailer-school links
   * GET /api/v1/retailer-schools/admin/pending
   */
  getAllPendingRequests = async (req, res) => {
    try {
      const result = await retailerSchoolService.getAllPendingRequests();
      ...
      logger.error("getAllPendingRequests controller error:", error);
      const statusCode = error.statusCode || 500;
      return res.status(statusCode).json({
        success: false,
        error: error.message,
  ...
  getConnectedRetailers = async (req, res) => {
    try {
      ...
      const result = await retailerSchoolService.getConnectedRetailers(
        schoolId,
        filters,
      );
