    ...
    async onboardRetailer(retailerId, data, signatureFile) {
        try {
            ...
            if (signatureFile) {
                ...
                const cleanName = signatureFile.originalname.replace(/[^a-zA-Z0-9.-]/g, "_");
    ...
    async checkRetailerDataStatus(retailerId) {
        ...
        if (!profile) {
            return {
                hasData: false,
                isComplete: false,
                missingFields: ["displayName", "ownerName", "gstin", "pan", "signatureUrl"],
                message: "Retailer profile not found. Please complete your onboarding.",
            };
        }

        // Check which required fields are missing
        const requiredFields = ["displayName", "ownerName", "gstin", "pan", "signatureUrl"];
