...
export class ImageService {
  ...
  async ensureBucketExists(bucketName) {
    ...
    try {
      ...
      if (error && error.message?.includes("not found")) {
        ...
        throw new AppError(
          `Failed to check storage bucket: ${error.message}`,
          500,
        );
      }
  ...
  async uploadImage(
    ...
    try {
      ...
      // structure: temp/TIMESTAMP-RANDOM-FILENAME.ext
    ...
    try {
      ...
      if (identifier.startsWith("http")) {
        ...
        if (urlParts.length === 2) {
          ...
          logger.warn(
            "Could not extract path from URL, assuming identifier is path or invalid URL",
          );
        }
      }
