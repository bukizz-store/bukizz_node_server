...
export class RetailerSchoolService {
  ...
  async linkRetailerToSchool({
    ...
    try {
      ...
      const existing = await this.retailerSchoolRepo.findByCompositeKey(
        retailerId,
        schoolId,
        status,
      );
      if (existing) {
        throw new AppError(
          `Retailer is already linked to this school with status '${status}'`,
          409,
        );
      }
  ...
  async getConnectedSchools(retailerId, filters = {}) {
    try {
      ...
      const records = await this.retailerSchoolRepo.findByRetailerId(
        retailerId,
        filters,
      );
  ...
  /**
   * Get all pending retailer-school requests globally (for Admin)
   * @returns {Object} { totalPending, requests }
   */
  async getAllPendingRequests() {
    try {
      const records = await this.retailerSchoolRepo.findAllPending();
      ...
      logger.error("getAllPendingRequests error:", error);
      throw error;
    }
  }
  ...
  async getConnectedRetailers(schoolId, filters = {}) {
    try {
      ...
      const records = await this.retailerSchoolRepo.findBySchoolId(
        schoolId,
        filters,
      );
  ...
  async updateLinkStatus(retailerId, schoolId, currentStatus, newStatus) {
    try {
      if (!retailerId || !schoolId || !currentStatus || !newStatus) {
        throw new AppError(
          "retailerId, schoolId, currentStatus, and newStatus are required",
          400,
        );
      }
      ...
      const updated = await this.retailerSchoolRepo.updateStatus(
        retailerId,
        schoolId,
        currentStatus,
        newStatus,
  ...
  async updateProductType(retailerId, schoolId, status, productType) {
    try {
      if (!retailerId || !schoolId || !status) {
        throw new AppError(
          "retailerId, schoolId, and status are required",
          400,
        );
      }
      ...
      const updated = await this.retailerSchoolRepo.updateProductType(
        retailerId,
        schoolId,
        status,
        productType,
  ...
  async unlinkRetailerFromSchool(retailerId, schoolId, status) {
    try {
      ...
      await this.retailerSchoolRepo.deleteByCompositeKey(
        retailerId,
        schoolId,
        status,
      );
